import React, { Component } from 'react';
import { Link } from 'react-router-dom';

class CompanyCard extends Component {
  static defaultProps = {
    defaultImg:
      'data:image/png;base64,'
  };
  render() {
    // create CSS style sheet and remove inline style
    // could destructure this.props to avoid repetition
    return (
      <Link to={`/companies/${this.props.handle}`}>
        <div className="CompanyCard card flex-row flex-wrap my-3">
          <div className="card-body">
            <h2 style={{ display: 'inline-flex' }}>{this.props.name}</h2>
            <img
              style={{ width: '50px', contentAlign: 'right' }}
              src={this.props.logo}
              alt="company logo"
            />
            <p>{this.props.desc}</p>
          </div>
        </div>
      </Link>
    );
  }
}

export default CompanyCard;
